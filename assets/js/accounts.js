...
const Accounts = {
    ...
    render: () => {
        ...
        // 1. Tarik seluruh data riwayat transaksi untuk dikalkulasi
        ...
        container.innerHTML = Accounts.data.map(acc => {
            // 2. Jadikan angka bawaan akun sebagai "Saldo Awal" (Initial Balance)
            let currentBalance = Number(acc.balance || 0);
            
            // 3. Kalkulasi Real-Time dari Riwayat Transaksi
            transactions.forEach(t => {
                const amount = Number(t.amount);
                
                // Jika akun ini adalah SUMBER transaksi (Uang Keluar / Pemasukan)
                if (t.account === acc.id) {
                    if (t.type === 'Income') currentBalance += amount;
                    if (t.type === 'Expense') currentBalance -= amount;
                    if (t.type === 'Transfer') currentBalance -= amount; // Uang ditarik dari akun ini
                }
                
                // Jika akun ini adalah TUJUAN transfer (Menerima Uang)
                if (t.type === 'Transfer' && t.toAccount === acc.id) {
                    currentBalance += amount; // Uang masuk ke akun ini
                }
            });

            const formattedBalance = Utils.formatCurrency(currentBalance, Accounts.currency);
            
            // Atur icon FontAwesome
            ...
            return `
                <div class="card item-card" style="--accent-color: ${acc.color};">
                    ...
                    <div class="item-balance" style="color: ${currentBalance < 0 ? 'var(--danger)' : 'var(--text-main)'}">
                        ${formattedBalance}
                    </div>
                </div>
            `;
