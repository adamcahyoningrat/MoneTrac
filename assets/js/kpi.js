...
const KPI = {
    ...
    getMonthlyData: (dateObj) => {
        ...
        KPI.transactions.forEach(t => {
            ...
            // 1. Pengecekan bulan & tahun yang akurat (Anti-Gagal Zona Waktu)
            if (d.getMonth() === targetMonth && d.getFullYear() === targetYear) {
                // 2. Kebal terhadap salah ketik (huruf besar/kecil/spasi)
                const txType = (t.type || '').trim().toLowerCase();
                
                if (txType === 'income') income += Number(t.amount);
                if (txType === 'expense') expense += Number(t.amount);
            }
        });
    ...
    calculateMetrics: () => {
        ...
        KPI.transactions.forEach(t => {
            const d = new Date(t.date);
            const txType = (t.type || '').trim().toLowerCase(); // Perbaikan tipe
            
            if (d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear() && txType === 'expense' && budgetCategories.includes(t.category)) {
